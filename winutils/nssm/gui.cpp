...
int nssm_gui(int resource, char *name, char *disp, char *desc) {
  char blurb[256];
  ...
  /* Set display name and description if given */
  if (disp) SetDlgItemText(dlg, IDC_DISPLAY, disp);
  if (desc) SetDlgItemText(dlg, IDC_DESCRIP, desc);
...
int install(HWND window) {
  ...
  /* Get display name, default to service-name if empty */
  if (SendMessage(GetDlgItem(window, IDC_DISPLAY), WM_GETTEXTLENGTH, 0, 0)) {
    GetDlgItemText(window, IDC_DISPLAY, disp, sizeof(disp));
  }
  else strcpy(disp, name);

  /* Get description, empty value is valid */
  if (SendMessage(GetDlgItem(window, IDC_DESCRIP), WM_GETTEXTLENGTH, 0, 0)) {
    GetDlgItemText(window, IDC_DESCRIP, desc, sizeof(desc));
  }
  else ZeroMemory(&desc, sizeof(desc));
  ...
  /* See if it works */
  switch (install_service(name, disp, desc, exe, flags)) {
    case 2:
    ...
    case 6:
      MessageBox(0, "Couldn't set description for the service!\nDeleting the service...", NSSM, MB_OK);
      return 6;
