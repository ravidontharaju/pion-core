...
const unsigned int		XMLLogServiceAppender::DEFAULT_MAX_EVENTS = 25;
...
XMLLogServiceAppender::XMLLogServiceAppender(void)
	: log4cpp::AppenderSkeleton("XMLLogServiceAppender"),
...
#if defined(PION_USE_LOG4CXX)
void XMLLogServiceAppender::append(const log4cxx::spi::LoggingEventPtr& event)
{
...
void XMLLogServiceAppender::append(const log4cplus::spi::InternalLoggingEvent& event)
{
	...
	std::string val (m_log_level_manager.toString(event.getLogLevel()));
	val += '/' + boost::lexical_cast<std::string>(event.getTimestamp().sec());
	val += '/' + event.getLoggerName();
...
#elif defined(PION_USE_LOG4CPP)
void XMLLogServiceAppender::_append(const log4cpp::LoggingEvent& event)
{
...
void XMLLogServiceAppender::addLogString(const std::string& log_string)
{
	boost::mutex::scoped_lock log_lock(m_log_mutex);
...
*/
	...
	for (LOG_QUEUE::reverse_iterator ir = m_log_event_queue.rbegin(); ir != m_log_event_queue.rend(); ++ir) {
		// FIXME: iterate reverse, and produce XML output, stuff it into writer via .str()
		xml << "<Event id=\"" << ir->first << "\">" << ConfigManager::xml_encode(ir->second)
			<< "</Event>";
	}
	xml << "</Events>";
	writer << xml.str();
